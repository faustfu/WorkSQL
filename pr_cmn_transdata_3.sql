if object_id('pr_cmn_transdata_3') is not null
    drop procedure pr_cmn_transdata_3
go
create proc [pr_cmn_transdata_3]
(
@Dt		datetime,		--作業日期
...
select @TableName = temptable_, @TitleNum = 1,
       @ColSplit = splitter_
       --,@ColSource = substring(selection_,patindex('%select %', selection_)+7,patindex('% from %', selection_)-patindex('%select %', selection_)-6)
  from msg_cmoney where pk_ = @FileNo;
...
--GOTO NORMAL_EXIT;
--==debug end
--read column names from the first row of data
select @ColSource = replace(replace(Data, '[', ''), ']', '')
  from @RowValue
 where Id = 1;
set @Cnt = 1;
declare ColCur cursor FAST_FORWARD for
select s.STR from dbo.fn_sys_splitstr_2(@ColSource,@ColSplit) s order by s.Id asc;
open ColCur;
...
begin
  ...
  select @TableSQL = @TableSQL + ', [' + @ColName + '] VARCHAR(100)';
  --next column
  select  @Cnt = @Cnt + 1;
  fetch next from ColCur into @ColName;
end
